...
class TestPrimitiveOp1(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([[[0.38741427659988403], [0.28904828429222107], [0.6316561102867126], [0.5805282592773438], [0.7266690731048584], [0.5417299866676331]]], dtype='float32').reshape([1, 6, 1]),
...
class TestPrimitiveOp3(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([[[0.4491495192050934], [0.1141708493232727], [0.45147964358329773], [0.660144031047821], [0.6789588332176208], [0.5037384629249573]]], dtype='float32').reshape([1, 6, 1]),
            paddle.to_tensor(3.0, dtype='float32').reshape([]),
        ]
        for input in self.inputs:
            input.stop_gradient = True
