...
class TestPrimitiveOp4(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([[[[0.024114154279232025, 0.12597566843032837]], [[0.0007672259816899896, 0.08278167992830276]], [[2.9555963919847272e-05, 0.39895984530448914]], [[0.2720852196216583, 0.06492781639099121]], [[0.19575229287147522, 0.332295686006546]], [[0.23373012244701385, 0.059741247445344925]]]], dtype='float32').reshape([1, 6, 1, 2]),
            paddle.to_tensor([-1], dtype='int64').reshape([1]),
        ]
        for input in self.inputs:
            input.stop_gradient = True
...
class TestPrimitiveOp5(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([[[[0.2014923095703125, 0.00024297228083014488]], [[0.006027367431670427, 0.007007616106420755]], [[0.14599892497062683, 0.05783496052026749]], [[0.11529609560966492, 0.32049405574798584]], [[0.040170907974243164, 0.4208141565322876]], [[0.002629258669912815, 0.25112318992614746]]]], dtype='float32').reshape([1, 6, 1, 2]),
            paddle.to_tensor([-1], dtype='int64').reshape([1]),
        ]
        for input in self.inputs:
            input.stop_gradient = True
...
class TestPrimitiveOp92(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([0.21596483886241913, -0.26111218333244324, -0.31223994493484497, -0.15396486222743988], dtype='float32').reshape([4]),
            paddle.to_tensor([], dtype='int64').reshape([]),
        ]
        for input in self.inputs:
            input.stop_gradient = True
...
class TestPrimitiveOp152(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([-0.07169418036937714, -0.2968689799308777, 0.023810334503650665, -0.14135989546775818, -0.16004933416843414, 0.08179563283920288, 0.19843006134033203, -0.13581593334674835, -0.0994613990187645, -0.05194544792175293, -0.1894889771938324, 0.009802241809666157, -0.24163924157619476, 0.08114949613809586, 0.2647280693054199, 0.2148674577474594, -0.07130137830972672, -0.19500359892845154, -0.17532818019390106, 0.19558367133140564], dtype='float32').reshape([20]),
            paddle.to_tensor([], dtype='int64').reshape([]),
        ]
        for input in self.inputs:
            input.stop_gradient = True
...
class TestPrimitiveOp166(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([-0.15925654768943787, -0.21066901087760925, 0.0394207164645195, -0.14328977465629578, 0.013441072776913643, -0.13358181715011597, 0.11361107230186462, -0.061719734221696854, -0.09407714754343033, -0.07037734240293503, -0.1413445919752121, -0.23733089864253998, 0.21535664796829224, -0.09234432876110077, -0.23285004496574402, 0.048052430152893066], dtype='float32').reshape([16]),
            paddle.to_tensor([], dtype='int64').reshape([]),
        ]
        for input in self.inputs:
            input.stop_gradient = True
...
class TestPrimitiveOp301(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([0.14257404208183289, 0.09787968546152115, 0.18271328508853912, -0.16638433933258057, -0.17203620076179504, -0.18101246654987335, 0.10852030664682388, 0.15675996243953705, -0.2601705491542816, -0.0625164732336998, 0.16189363598823547, -0.3035373091697693, -0.1623634696006775, -0.00402759900316596, -0.012948683463037014, 0.029563086107373238, -0.15542757511138916, -0.1406424343585968, 0.17690126597881317, 0.18002326786518097, 0.03696097433567047, 0.2302350550889969, 0.25513893365859985, 0.1968325972557068], dtype='float32').reshape([24]),
            paddle.to_tensor([], dtype='int64').reshape([]),
        ]
        for input in self.inputs:
            input.stop_gradient = True
