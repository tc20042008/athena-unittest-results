...
class TestPrimitiveOp0(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([[[-0.3681713342666626, 0.4185675382614136, 0.3345829248428345, -0.2264307737350464]]], dtype='float32').reshape([1, 1, 4]),
...
class TestPrimitiveOp5(CinnTestBase, unittest.TestCase):
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([[[-0.018948793411254883, -0.20550090074539185, -0.07790839672088623, 0.08290481567382812]]], dtype='float32').reshape([1, 1, 4]),
