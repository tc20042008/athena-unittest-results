...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, memory_efficient_attention_0):
        ...
        #  type: (-1x-1x8x80xf32) <- (-1x-1x8x80xf16)
        # shape: ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 8, 80]) <- ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 8, 80])
        #  data: (None) <- (None)
        ...
        #  type: (-1x-1x8x80xf16) <- (-1x-1x8x80xf32)
        # shape: ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 8, 80]) <- ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 8, 80])
        #  data: (None) <- (None)
        ...
        #  type: (-1x-1x640xf16) <- (-1x-1x8x80xf16)
        # shape: ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 640]) <- ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 8, 80])
        #  data: (None) <- (None)
        reshape_0 = paddle.reshape(cast_1, [0, 0, 640])
...
class TestFusionOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            paddle.uniform([2, 2, 8, 80], dtype='float16', min=-0.5, max=0.5),
        ]
    ...
    def apply_to_static(self, net, use_cinn):
        ...
        input_spec = [
            paddle.static.InputSpec(shape=[None, None, 8, 80], dtype='float16'),
        ]
