...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, matmul_0, parameter_0, fusion_0, fusion_1):
        ...
        #  type: (3xi64) <- (-1x-1x320xf16)
        # shape: ([3]) <- ([S0, S3*S3, 320])
        #  data: ([S0, S3*S3, 320]) <- (None)
        generate_shape_0 = [matmul_0.shape[0], matmul_0.shape[1], 320] # inputs: matmul_0

        if FastReturn(1):
            return matmul_0, parameter_0, fusion_0, fusion_1, generate_shape_0

        #  type: (-1x-1x-1xf16) <- (320xf16, 3xi64)
        ...
        #  data: ([S0, S3, S3, 320]) <- (None)
        generate_shape_1 = [fusion_1.shape[0], fusion_1.shape[3], fusion_1.shape[3], 320] # inputs: fusion_1
