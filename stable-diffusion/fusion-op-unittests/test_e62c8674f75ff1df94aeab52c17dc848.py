...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0):
        ...
        #  type: (-1x-1x-1x640xf16) <- (-1x640x-1x-1xf16)
        # shape: ([S0, (S3-1)/2+1, (S3-1)/2+1, 640]) <- ([S0, 640, (S3-1)/2+1, (S3-1)/2+1])
        #  data: (None) <- (None)
        ...
        #  type: (3xi64) <- (-1x-1x-1x640xf16)
        # shape: ([3]) <- ([S0, (S3-1)/2+1, (S3-1)/2+1, 640])
        #  data: ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 640]) <- (None)
        generate_shape_1 = [transpose_0.shape[0], (transpose_0.shape[2] * transpose_0.shape[2]), 640] # inputs: transpose_0

        if FastReturn(6):
            return transpose_0, generate_shape_1

        #  type: (-1x-1x-1xf16, 0x-1x-1x-1x640xi64) <- (-1x-1x-1x640xf16, 3xi64)
        # shape: ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 640], [0, S0, (S3-1)/2+1, (S3-1)/2+1, 640]) <- ([S0, (S3-1)/2+1, (S3-1)/2+1, 640], [3])
