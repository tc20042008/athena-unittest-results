...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, arange_0, fusion_0, argmax_0):

        if FastReturn(0):
            return arange_0, fusion_0, argmax_0

        #  type: (2xi64) <- (-1xi32, -1x-1x768xf16)
        # shape: ([2]) <- ([S0], [S0, S1, 768])
        #  data: ([S0, 1]) <- (None, None)
        generate_shape_0 = [fusion_0.shape[0], 1] # inputs: arange_0, fusion_0

        if FastReturn(1):
            return arange_0, fusion_0, argmax_0, generate_shape_0
        ...
        if FastReturn(2):
            return fusion_0, argmax_0, generate_shape_0, reshape_0
        ...
        if FastReturn(3):
            return fusion_0, reshape_0, reshape_2
        ...
        if FastReturn(4):
            return fusion_0, concat_0

        #  type: (-1x768xf16) <- (-1x-1x768xf16, -1x-1xi32)
        # shape: ([S0, 768]) <- ([S0, S1, 768], [S0, 2])
        #  data: (None) <- (None, None)
        gather_nd_0 = paddle.gather_nd(fusion_0, concat_0)
