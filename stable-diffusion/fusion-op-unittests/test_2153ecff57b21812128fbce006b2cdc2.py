...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0):
        ...
        # shape: ([S0, S6, S6, 640]) <- ([S0, 640, S6, S6])
        #  data: (None) <- (None)
        ...
        #  type: (3xi64) <- (-1x640x-1x-1xf16)
        # shape: ([3]) <- ([S0, 640, S6, S6])
        #  data: ([S0, S6*S6, 640]) <- (None)
        generate_shape_1 = [conv2d_0.shape[0], (conv2d_0.shape[3] * conv2d_0.shape[3]), 640] # inputs: conv2d_0

        if FastReturn(6):
            return transpose_0, generate_shape_1

        #  type: (-1x-1x-1xf16, 0x-1x-1x-1x640xi64) <- (-1x-1x-1x640xf16, 3xi64)
        # shape: ([S0, S6*S6, 640], [0, S0, S6, S6, 640]) <- ([S0, S6, S6, 640], [3])
        ...
        # shape: ([S0, S6*S6, 640]) <- ([S0, S6*S6, 640])
        #  data: (None) <- (None)
