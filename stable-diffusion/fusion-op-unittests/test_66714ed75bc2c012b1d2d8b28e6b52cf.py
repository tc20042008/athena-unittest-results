...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, matmul_0, parameter_0):
        ...
        #  type: (3xi64) <- (-1x-1x2560xf16)
        # shape: ([3]) <- ([S0, S3*S3, 2560])
        #  data: ([S0, S3*S3, 2560]) <- (None)
        generate_shape_0 = [matmul_0.shape[0], matmul_0.shape[1], 2560] # inputs: matmul_0

        if FastReturn(1):
            return matmul_0, parameter_0, generate_shape_0

        #  type: (-1x-1x-1xf16) <- (2560xf16, 3xi64)
        # shape: ([S0, S3*S3, 2560]) <- ([2560], [3])
        #  data: (None) <- (None, [S0, S3*S3, 2560])
        expand_0 = paddle.expand(parameter_0, generate_shape_0)

        if FastReturn(2):
            return matmul_0, expand_0

        #  type: (-1x-1x2560xf16) <- (-1x-1x2560xf16, -1x-1x-1xf16)
        # shape: ([S0, S3*S3, 2560]) <- ([S0, S3*S3, 2560], [S0, S3*S3, 2560])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x-1x1280xf16) <- (-1x-1x2560xf16)
        # shape: ([S0, S3*S3, 1280]) <- ([S0, S3*S3, 2560])
        #  data: (None) <- (None)
        slice_0 = paddle.slice(add_0, axes=[2], starts=[1280], ends=[2560])
        ...
        # shape: ([S0, S3*S3, 1280]) <- ([S0, S3*S3, 2560])
        #  data: (None) <- (None)
        slice_1 = paddle.slice(add_0, axes=[2], starts=[0], ends=[1280])
        ...
        # shape: ([3]) <- ([S0, S3*S3, 1280])
        #  data: ([S0, S3*S3, 1280]) <- (None)
        generate_shape_1 = [slice_0.shape[0], slice_0.shape[1], 1280] # inputs: slice_0

        if FastReturn(9):
            return slice_0, slice_1, full_0, full_1, full_2, generate_shape_1

        #  type: (-1x-1x-1xf16) <- (xf16, 3xi64)
        # shape: ([S0, S3*S3, 1280]) <- ([], [3])
        #  data: (None) <- ([0], [S0, S3*S3, 1280])
        ...
        # shape: ([S0, S3*S3, 1280]) <- ([S0, S3*S3, 1280], [S0, S3*S3, 1280])
        #  data: (None) <- (None, None)
        ...
        # shape: ([S0, S3*S3, 1280]) <- ([S0, S3*S3, 1280])
        #  data: (None) <- (None)
        ...
        # shape: ([3]) <- ([S0, S3*S3, 1280])
        #  data: ([S0, S3*S3, 1280]) <- (None)
        generate_shape_2 = [erf_0.shape[0], erf_0.shape[1], 1280] # inputs: erf_0
        ...
        # shape: ([3]) <- ([S0, S3*S3, 1280])
        #  data: ([S0, S3*S3, 1280]) <- (None)
        generate_shape_3 = [slice_0.shape[0], slice_0.shape[1], 1280] # inputs: slice_0
...
class TestFusionOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            paddle.uniform([2, 2, 2560], dtype='float16', min=-0.5, max=0.5),
    ...
    def apply_to_static(self, net, use_cinn):
        ...
        input_spec = [
            paddle.static.InputSpec(shape=[None, None, 2560], dtype='float16'),
