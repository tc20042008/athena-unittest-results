...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0, parameter_1, conv2d_1, fusion_0):

        if FastReturn(0):
            return parameter_0, conv2d_0, parameter_1, conv2d_1, fusion_0
        ...
        if FastReturn(1):
            return conv2d_0, parameter_1, conv2d_1, fusion_0, reshape_0
        ...
        if FastReturn(2):
            return conv2d_0, parameter_1, conv2d_1, fusion_0, reshape_0, generate_shape_0
        ...
        if FastReturn(3):
            return conv2d_0, parameter_1, conv2d_1, fusion_0, expand_0
        ...
        if FastReturn(4):
            return parameter_1, conv2d_1, fusion_0, add_0
        ...
        if FastReturn(5):
            return conv2d_1, fusion_0, add_0, reshape_1

        #  type: (4xi64) <- (-1x1280x-1x-1xf16)
        # shape: ([4]) <- ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1])
        #  data: ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1]) <- (None)
        generate_shape_1 = [conv2d_1.shape[0], 1280, conv2d_1.shape[3], conv2d_1.shape[3]] # inputs: conv2d_1

        if FastReturn(6):
            return conv2d_1, fusion_0, add_0, reshape_1, generate_shape_1
        ...
        if FastReturn(7):
            return conv2d_1, fusion_0, add_0, expand_1
        ...
        #  data: (None) <- (None, None)
        concat_0 = paddle.concat([scale_0, fusion_0], axis=1)

        if FastReturn(11):
            return concat_0

        #  type: (-1x2560x-1x-1xf16) <- (-1x2560x-1x-1xf16)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: (None) <- (None)
        yield_store_0 = concat_0

        if FastReturn(12):
            return concat_0, yield_store_0
