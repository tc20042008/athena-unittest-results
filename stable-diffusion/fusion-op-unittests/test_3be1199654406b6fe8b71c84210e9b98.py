...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0, parameter_1, matmul_0):
        ...
        # shape: ([S0, 1280, S5, S5]) <- ([1, 1280, 1, 1], [4])
        #  data: (None) <- (None, [S0, 1280, S5, S5])
        ...
        # shape: ([1, 1280]) <- ([1280])
        #  data: (None) <- (None)
        broadcast_0 = paddle.broadcast_to(parameter_1, [1, 1280])
        ...
        #  type: (1x1280x1x1xf16) <- (1x1280xf16)
        # shape: ([1, 1280, 1, 1]) <- ([1, 1280])
        #  data: (None) <- (None)
        reshape_1 = paddle.reshape(add_1, [1, 1280, 1, 1])
        ...
        # shape: ([4]) <- ([S0, 1280, S5, S5])
        #  data: ([S0, 1280, S5, S5]) <- (None)
        generate_shape_1 = [conv2d_0.shape[0], 1280, conv2d_0.shape[3], conv2d_0.shape[3]] # inputs: conv2d_0
