...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, fusion_0, fusion_1, parameter_0, parameter_1):
        ...
        #  type: (-1x-1x320xf32) <- (-1x-1x320xf16)
        ...
        # shape: ([]) <- ()
        #  data: ([320]) <- ()
        full_0 = paddle.full(shape=[], dtype='float32', fill_value=320)
        ...
        # shape: ([S0, S3*S3, 1]) <- ([S0, S3*S3, 1], [S0, S3*S3, 1])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x-1x320xf32) <- (-1x-1x320xf32, -1x-1x320xf32)
        # shape: ([S0, S3*S3, 320]) <- ([S0, S3*S3, 320], [S0, S3*S3, 320])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x-1x1xf32) <- (-1x-1x320xf32)
        # shape: ([S0, S3*S3, 1]) <- ([S0, S3*S3, 320])
        #  data: (None) <- (None)
        ...
        # shape: ([]) <- ()
        #  data: ([320]) <- ()
        full_1 = paddle.full(shape=[], dtype='float32', fill_value=320)
        ...
        # shape: ([3]) <- ([S0, S3*S3, 320])
        #  data: ([S0, S3*S3, 320]) <- (None)
        generate_shape_4 = [subtract_0.shape[0], subtract_0.shape[1], 320] # inputs: subtract_0
        ...
        # shape: ([3]) <- ([S0, S3*S3, 320])
        #  data: ([S0, S3*S3, 320]) <- (None)
        generate_shape_5 = [multiply_1.shape[0], multiply_1.shape[1], 320] # inputs: multiply_1

        if FastReturn(24):
            return parameter_1, multiply_1, cast_1, generate_shape_5

        #  type: (-1x-1x-1xf32) <- (-1xf32, 3xi64)
        # shape: ([S0, S3*S3, 320]) <- ([320], [3])
        #  data: (None) <- (None, [S0, S3*S3, 320])
        ...
        # shape: ([3]) <- ([S0, S3*S3, 320])
        #  data: ([S0, S3*S3, 320]) <- (None)
        generate_shape_6 = [multiply_2.shape[0], multiply_2.shape[1], 320] # inputs: multiply_2
        ...
        #  type: (-1x-1x320xf16) <- (-1x-1x320xf32)
        # shape: ([S0, S3*S3, 320]) <- ([S0, S3*S3, 320])
        #  data: (None) <- (None)
...
class TestFusionOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            paddle.uniform([2, 2, 320], dtype='float16', min=-0.5, max=0.5),
