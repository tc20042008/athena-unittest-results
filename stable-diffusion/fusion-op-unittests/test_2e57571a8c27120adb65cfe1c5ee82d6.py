...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, fusion_0, fusion_1, fusion_2, fusion_3, fusion_4, fusion_5, parameter_0, parameter_1):
        ...
        #  type: (2xi64) <- (-1x-1xf32)
        # shape: ([2]) <- ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80])
        #  data: ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80]) <- (None)
        ...
        # shape: ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80]) <- ([S0*32, 1], [2])
        #  data: (None) <- (None, [S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80])
        ...
        #  type: (-1x-1xf32) <- (-1x-1xf32, -1x-1xf32)
        # shape: ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80]) <- ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80], [S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80])
        #  data: (None) <- (None, None)
        ...
        #  type: (4xi64) <- (-1x2560x-1x-1xf32)
        # shape: ([4]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- (None)
        generate_shape_6 = [fusion_5.shape[0], 2560, fusion_5.shape[3], fusion_5.shape[3]] # inputs: fusion_5

        if FastReturn(24):
            return parameter_0, parameter_1, multiply_1, generate_shape_6

        #  type: (-1x-1x-1x-1xf32, 0x-1x-1xi64) <- (-1x-1xf32, 4xi64)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1], [0, S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80]) <- ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80], [4])
        #  data: (None, None) <- (None, [S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        reshape_0, reshape_1 = paddle.reshape(multiply_1, generate_shape_6), None

        if FastReturn(25):
            return parameter_0, parameter_1, reshape_0

        #  type: (2560x1x1xf16) <- (2560xf16)
        # shape: ([2560, 1, 1]) <- ([2560])
        #  data: (None) <- (None)
        ...
        #  type: (2560x1x1xf32) <- (2560x1x1xf16)
        # shape: ([2560, 1, 1]) <- ([2560, 1, 1])
        #  data: (None) <- (None)
        ...
        # shape: ([4]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- (None)
        generate_shape_7 = [reshape_0.shape[0], 2560, reshape_0.shape[3], reshape_0.shape[3]] # inputs: reshape_0

        if FastReturn(28):
            return parameter_1, reshape_0, cast_0, generate_shape_7

        #  type: (-1x-1x-1x-1xf32) <- (2560x1x1xf32, 4xi64)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([2560, 1, 1], [4])
        #  data: (None) <- (None, [S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        expand_6 = paddle.expand(cast_0, generate_shape_7)

        if FastReturn(29):
            return parameter_1, reshape_0, expand_6

        #  type: (-1x2560x-1x-1xf32) <- (-1x-1x-1x-1xf32, -1x-1x-1x-1xf32)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1], [S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([4]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- (None)
        generate_shape_8 = [multiply_2.shape[0], 2560, multiply_2.shape[3], multiply_2.shape[3]] # inputs: multiply_2
        ...
        #  type: (-1x2560x-1x-1xf32) <- (-1x2560x-1x-1xf32, -1x-1x-1x-1xf32)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1], [S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x2560x-1x-1xf16) <- (-1x2560x-1x-1xf32)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: (None) <- (None)
        ...
        #  type: (-1x2560x-1x-1xf32) <- (-1x2560x-1x-1xf32)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: (None) <- (None)
        scale_0 = cast_3 * -1 + 0
        ...
        # shape: ([4]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- (None)
        generate_shape_9 = [exp_0.shape[0], 2560, exp_0.shape[3], exp_0.shape[3]] # inputs: exp_0

        if FastReturn(41):
            return cast_3, full_2, exp_0, generate_shape_9

        #  type: (-1x-1x-1x-1xf32) <- (1xf32, 4xi64)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([1], [4])
        #  data: (None) <- ([1], [S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        expand_8 = paddle.expand(full_2, generate_shape_9)

        if FastReturn(42):
            return cast_3, full_2, exp_0, expand_8

        #  type: (-1x2560x-1x-1xf32) <- (-1x-1x-1x-1xf32, -1x2560x-1x-1xf32)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1], [S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([4]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- (None)
        generate_shape_10 = [add_2.shape[0], 2560, add_2.shape[3], add_2.shape[3]] # inputs: add_2
        ...
        #  type: (-1x2560x-1x-1xf32) <- (-1x2560x-1x-1xf32, -1x2560x-1x-1xf32)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1], [S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: (None) <- (None, None)
        ...
        #  type: () <- (-1x2560x-1x-1xf16)
        # shape: () <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: () <- (None)
        return cast_4
...
class TestFusionOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            ...
            paddle.uniform([64, 1], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([64, 320], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([2, 2560, 2, 2], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([2560], dtype='float16', min=-0.5, max=0.5),
            paddle.uniform([2560], dtype='float16', min=-0.5, max=0.5),
        ]
