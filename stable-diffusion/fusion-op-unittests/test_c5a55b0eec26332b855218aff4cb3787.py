...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, fusion_0, fusion_1, fusion_2, fusion_3, fusion_4, fusion_5, parameter_0, parameter_1):
        ...
        #  type: (2xi64) <- (-1x-1xf32)
        # shape: ([2]) <- ([S0*32, S6*S6*30])
        #  data: ([S0*32, S6*S6*30]) <- (None)
        ...
        # shape: ([S0*32, S6*S6*30]) <- ([S0*32, 1], [2])
        #  data: (None) <- (None, [S0*32, S6*S6*30])
        ...
        #  type: (-1x-1xf32) <- (-1x-1xf32, -1x-1xf32)
        # shape: ([S0*32, S6*S6*30]) <- ([S0*32, S6*S6*30], [S0*32, S6*S6*30])
        #  data: (None) <- (None, None)
        ...
        # shape: ([2]) <- ([S0, 960, S6, S6])
        #  data: ([S0*32, S6*S6*30]) <- (None)
        generate_shape_5 = [(fusion_0.shape[0] * 32), (fusion_0.shape[3] * fusion_0.shape[3] * 30)] # inputs: fusion_0
        ...
        #  type: (4xi64) <- (-1x960x-1x-1xf32)
        # shape: ([4]) <- ([S0, 960, S6, S6])
        #  data: ([S0, 960, S6, S6]) <- (None)
        generate_shape_6 = [fusion_0.shape[0], 960, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0

        if FastReturn(24):
            return fusion_0, parameter_0, parameter_1, multiply_1, generate_shape_6

        #  type: (-1x-1x-1x-1xf32, 0x-1x-1xi64) <- (-1x-1xf32, 4xi64)
        # shape: ([S0, 960, S6, S6], [0, S0*32, S6*S6*30]) <- ([S0*32, S6*S6*30], [4])
        #  data: (None, None) <- (None, [S0, 960, S6, S6])
        reshape_0, reshape_1 = paddle.reshape(multiply_1, generate_shape_6), None

        if FastReturn(25):
            return fusion_0, parameter_0, parameter_1, reshape_0

        #  type: (960x1x1xf16) <- (960xf16)
        # shape: ([960, 1, 1]) <- ([960])
        #  data: (None) <- (None)
        ...
        #  type: (960x1x1xf32) <- (960x1x1xf16)
        # shape: ([960, 1, 1]) <- ([960, 1, 1])
        #  data: (None) <- (None)
        ...
        # shape: ([4]) <- ([S0, 960, S6, S6])
        #  data: ([S0, 960, S6, S6]) <- (None)
        generate_shape_7 = [fusion_0.shape[0], 960, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0

        if FastReturn(28):
            return fusion_0, parameter_1, reshape_0, cast_0, generate_shape_7

        #  type: (-1x-1x-1x-1xf32) <- (960x1x1xf32, 4xi64)
        # shape: ([S0, 960, S6, S6]) <- ([960, 1, 1], [4])
        #  data: (None) <- (None, [S0, 960, S6, S6])
        expand_6 = paddle.expand(cast_0, generate_shape_7)

        if FastReturn(29):
            return fusion_0, parameter_1, reshape_0, expand_6

        #  type: (-1x960x-1x-1xf32) <- (-1x-1x-1x-1xf32, -1x-1x-1x-1xf32)
        # shape: ([S0, 960, S6, S6]) <- ([S0, 960, S6, S6], [S0, 960, S6, S6])
        #  data: (None) <- (None, None)
        ...
        # shape: ([4]) <- ([S0, 960, S6, S6])
        #  data: ([S0, 960, S6, S6]) <- (None)
        generate_shape_8 = [fusion_0.shape[0], 960, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0
        ...
        #  type: (-1x960x-1x-1xf32) <- (-1x960x-1x-1xf32, -1x-1x-1x-1xf32)
        # shape: ([S0, 960, S6, S6]) <- ([S0, 960, S6, S6], [S0, 960, S6, S6])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x960x-1x-1xf16) <- (-1x960x-1x-1xf32)
        # shape: ([S0, 960, S6, S6]) <- ([S0, 960, S6, S6])
        #  data: (None) <- (None)
        ...
        #  type: (-1x960x-1x-1xf32) <- (-1x960x-1x-1xf32)
        # shape: ([S0, 960, S6, S6]) <- ([S0, 960, S6, S6])
        #  data: (None) <- (None)
        scale_0 = cast_3 * -1 + 0
        ...
        # shape: ([4]) <- ([S0, 960, S6, S6])
        #  data: ([S0, 960, S6, S6]) <- (None)
        generate_shape_9 = [fusion_0.shape[0], 960, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0

        if FastReturn(41):
            return fusion_0, cast_3, full_2, exp_0, generate_shape_9

        #  type: (-1x-1x-1x-1xf32) <- (1xf32, 4xi64)
        # shape: ([S0, 960, S6, S6]) <- ([1], [4])
        #  data: (None) <- ([1], [S0, 960, S6, S6])
        expand_8 = paddle.expand(full_2, generate_shape_9)

        if FastReturn(42):
            return fusion_0, cast_3, full_2, exp_0, expand_8

        #  type: (-1x960x-1x-1xf32) <- (-1x-1x-1x-1xf32, -1x960x-1x-1xf32)
        # shape: ([S0, 960, S6, S6]) <- ([S0, 960, S6, S6], [S0, 960, S6, S6])
        #  data: (None) <- (None, None)
        ...
        # shape: ([4]) <- ([S0, 960, S6, S6])
        #  data: ([S0, 960, S6, S6]) <- (None)
        generate_shape_10 = [fusion_0.shape[0], 960, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0
        ...
        #  type: (-1x960x-1x-1xf32) <- (-1x960x-1x-1xf32, -1x960x-1x-1xf32)
        # shape: ([S0, 960, S6, S6]) <- ([S0, 960, S6, S6], [S0, 960, S6, S6])
        #  data: (None) <- (None, None)
...
class TestFusionOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            paddle.uniform([2, 960, 2, 2], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([64, 1], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([64, 1], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([64, 120], dtype='float32', min=-0.5, max=0.5),
            paddle.uniform([960], dtype='float16', min=-0.5, max=0.5),
            paddle.uniform([960], dtype='float16', min=-0.5, max=0.5),
        ]
