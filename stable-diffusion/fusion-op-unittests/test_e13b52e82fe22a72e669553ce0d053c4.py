...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, fusion_0, fusion_1, fusion_2, fusion_3, fusion_4, fusion_5, parameter_0, parameter_1):
        ...
        #  type: (2xi64) <- (-1x640x-1x-1xf32)
        # shape: ([2]) <- ([S0, 640, S3, S3])
        #  data: ([S0*32, 1]) <- (None)
        ...
        #  type: (2xi64) <- (-1x-1xf32)
        # shape: ([2]) <- ([S0*32, S3*S3*20])
        #  data: ([S0*32, S3*S3*20]) <- (None)
        ...
        # shape: ([S0*32, S3*S3*20]) <- ([S0*32, 1], [2])
        #  data: (None) <- (None, [S0*32, S3*S3*20])
        ...
        #  type: (-1x-1xf32) <- (-1x-1xf32, -1x-1xf32)
        # shape: ([S0*32, S3*S3*20]) <- ([S0*32, S3*S3*20], [S0*32, S3*S3*20])
        #  data: (None) <- (None, None)
        ...
        # shape: ([2]) <- ([S0, 640, S3, S3])
        #  data: ([S0*32, S3*S3*20]) <- (None)
        generate_shape_5 = [(fusion_0.shape[0] * 32), (fusion_0.shape[3] * fusion_0.shape[3] * 20)] # inputs: fusion_0
        ...
        # shape: ([4]) <- ([S0, 640, S3, S3])
        #  data: ([S0, 640, S3, S3]) <- (None)
        generate_shape_6 = [fusion_0.shape[0], 640, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0

        if FastReturn(24):
            return fusion_0, parameter_0, parameter_1, multiply_1, generate_shape_6

        #  type: (-1x-1x-1x-1xf32, 0x-1x-1xi64) <- (-1x-1xf32, 4xi64)
        # shape: ([S0, 640, S3, S3], [0, S0*32, S3*S3*20]) <- ([S0*32, S3*S3*20], [4])
        #  data: (None, None) <- (None, [S0, 640, S3, S3])
        ...
        # shape: ([4]) <- ([S0, 640, S3, S3])
        #  data: ([S0, 640, S3, S3]) <- (None)
        generate_shape_7 = [fusion_0.shape[0], 640, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0

        if FastReturn(28):
            return fusion_0, parameter_1, reshape_0, cast_0, generate_shape_7

        #  type: (-1x-1x-1x-1xf32) <- (640x1x1xf32, 4xi64)
        # shape: ([S0, 640, S3, S3]) <- ([640, 1, 1], [4])
        #  data: (None) <- (None, [S0, 640, S3, S3])
        ...
        # shape: ([S0, 640, S3, S3]) <- ([S0, 640, S3, S3], [S0, 640, S3, S3])
        #  data: (None) <- (None, None)
        ...
        # shape: ([4]) <- ([S0, 640, S3, S3])
        #  data: ([S0, 640, S3, S3]) <- (None)
        generate_shape_8 = [fusion_0.shape[0], 640, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0
        ...
        # shape: ([S0, 640, S3, S3]) <- ([S0, 640, S3, S3])
        #  data: (None) <- (None)
        ...
        # shape: ([4]) <- ([S0, 640, S3, S3])
        #  data: ([S0, 640, S3, S3]) <- (None)
        generate_shape_9 = [fusion_0.shape[0], 640, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0

        if FastReturn(41):
            return fusion_0, cast_3, full_2, exp_0, generate_shape_9

        #  type: (-1x-1x-1x-1xf32) <- (1xf32, 4xi64)
        # shape: ([S0, 640, S3, S3]) <- ([1], [4])
        #  data: (None) <- ([1], [S0, 640, S3, S3])
        ...
        # shape: ([4]) <- ([S0, 640, S3, S3])
        #  data: ([S0, 640, S3, S3]) <- (None)
        generate_shape_10 = [fusion_0.shape[0], 640, fusion_0.shape[3], fusion_0.shape[3]] # inputs: fusion_0
