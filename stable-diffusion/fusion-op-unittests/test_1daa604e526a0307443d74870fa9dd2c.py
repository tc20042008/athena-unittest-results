...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, matmul_0, parameter_0, fusion_0, fusion_1):
        ...
        # shape: ([S0, (S3-1)/8+1, (S3-1)/8+1, 1280], [0, S0, ((S3-1)/8+1)*((S3-1)/8+1), 1280]) <- ([S0, ((S3-1)/8+1)*((S3-1)/8+1), 1280], [4])
        #  data: (None, None) <- (None, [S0, (S3-1)/8+1, (S3-1)/8+1, 1280])
        ...
        # shape: ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, (S3-1)/8+1, (S3-1)/8+1, 1280])
        #  data: (None) <- (None)
