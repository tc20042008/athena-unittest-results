...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, feed_0, arange_0):
        ...
        #  type: (1x160xf16) <- (1x1xf16)
        # shape: ([1, 160]) <- ([1, 1])
        #  data: (None) <- (None)
        broadcast_1 = paddle.broadcast_to(cast_3, [1, 160])

        if FastReturn(12):
            return reshape_1, broadcast_1

        #  type: (1x160xf16) <- (1x160xf16, 1x160xf16)
        # shape: ([1, 160]) <- ([1, 160], [1, 160])
        #  data: (None) <- (None, None)
        multiply_0 = broadcast_1 * reshape_1
