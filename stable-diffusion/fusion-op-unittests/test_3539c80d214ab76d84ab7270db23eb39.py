...
class FusionOp(paddle.nn.Layer):
    ...
    def forward(self, fusion_0, fusion_1, fusion_2, parameter_0, parameter_1):
        ...
        # shape: ([S0, S6*S6, 1]) <- ([S0, S6*S6, 1], [S0, S6*S6, 1])
        #  data: (None) <- (None, None)
        ...
        generate_shape_2 = [fusion_2.shape[0], fusion_2.shape[1], 640] # inputs: fusion_2

        if FastReturn(10):
            return fusion_2, parameter_0, parameter_1, rsqrt_0, generate_shape_2

        #  type: (-1x-1x-1xf32) <- (-1x-1x1xf32, 3xi64)
        # shape: ([S0, S6*S6, 640]) <- ([S0, S6*S6, 1], [3])
        ...
        # shape: ([3]) <- ([S0, S6*S6, 640])
        #  data: ([S0, S6*S6, 640]) <- (None)
        generate_shape_3 = [multiply_0.shape[0], multiply_0.shape[1], 640] # inputs: multiply_0
        ...
        # shape: ([3]) <- ([S0, S6*S6, 640])
        #  data: ([S0, S6*S6, 640]) <- (None)
        generate_shape_4 = [multiply_1.shape[0], multiply_1.shape[1], 640] # inputs: multiply_1
