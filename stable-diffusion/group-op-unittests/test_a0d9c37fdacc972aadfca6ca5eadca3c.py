...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, embedding_0, embedding_1, parameter_0, parameter_1):

        if FastReturn(0):
            return embedding_0, embedding_1, parameter_0, parameter_1

        #  type: (-1x-1x768xf16) <- (-1x-1x768xf16, 1x-1x768xf16)
        # shape: ([S0, S1, 768]) <- ([S0, S1, 768], [1, S1, 768])
        #  data: (None) <- (None, None)
        add_0 = embedding_0 + embedding_1

        if FastReturn(1):
            return parameter_0, parameter_1, add_0
        ...
        if FastReturn(2):
            return parameter_0, parameter_1, add_0, cast_0
        ...
        if FastReturn(3):
            return parameter_0, parameter_1, add_0, cast_0, reduce_sum_0
        ...
        if FastReturn(4):
            return parameter_0, parameter_1, add_0, cast_0, reduce_sum_0, full_0
        ...
        if FastReturn(5):
            return parameter_0, parameter_1, add_0, cast_0, divide_0
        ...
        if FastReturn(6):
            return parameter_0, parameter_1, add_0, subtract_0
        ...
        if FastReturn(7):
            return parameter_0, parameter_1, add_0, subtract_0, multiply_0
        ...
        if FastReturn(8):
            return parameter_0, parameter_1, add_0, subtract_0, reduce_sum_1
        ...
        if FastReturn(9):
            return parameter_0, parameter_1, add_0, subtract_0, reduce_sum_1, full_1
        ...
        if FastReturn(10):
            return parameter_0, parameter_1, add_0, subtract_0, divide_1
        ...
        if FastReturn(11):
            return parameter_0, parameter_1, add_0, subtract_0, divide_1, full_2

        #  type: (-1x-1x1xf32) <- (-1x-1x1xf32, xf32)
        # shape: ([S0, S1, 1]) <- ([S0, S1, 1], [])
        #  data: (None) <- (None, [0])
        add_1 = divide_1 + full_2

        if FastReturn(12):
            return parameter_0, parameter_1, add_0, subtract_0, add_1

        #  type: (-1x-1x1xf32) <- (-1x-1x1xf32)
        # shape: ([S0, S1, 1]) <- ([S0, S1, 1])
        #  data: (None) <- (None)
        rsqrt_0 = paddle.rsqrt(add_1)

        if FastReturn(13):
            return parameter_0, parameter_1, add_0, subtract_0, rsqrt_0
        ...
        if FastReturn(14):
            return parameter_0, parameter_1, add_0, multiply_1

        #  type: (1xi64) <- (-1x-1x768xf16)
        # shape: ([1]) <- ([S0, S1, 768])
        #  data: ([768]) <- (None)
        generate_shape_0 = [768] # inputs: add_0
        ...
        if FastReturn(16):
            return parameter_1, add_0, multiply_1, cast_1
        ...
        if FastReturn(17):
            return parameter_1, add_0, multiply_2
        ...
        if FastReturn(18):
            return add_0, multiply_2, cast_2

        #  type: (-1x-1x768xf32) <- (-1x-1x768xf32, -1xf32)
        # shape: ([S0, S1, 768]) <- ([S0, S1, 768], [768])
        #  data: (None) <- (None, None)
        add_2 = multiply_2 + cast_2
        ...
        # shape: ([S0, S1, 768]) <- ([S0, S1, 768])
        #  data: (None) <- (None)
        cast_3 = paddle.cast(add_2, dtype='float16')
...
class TestGroupOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            ...
            paddle.uniform([1, 2, 768], dtype='float16', min=-0.5, max=0.5),
    ...
    def apply_to_static(self, net, use_cinn):
        ...
        input_spec = [
            ...
            paddle.static.InputSpec(shape=[1, None, 768], dtype='float16'),
