...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, group_0, full_int_array_0, shape_0, group_1, full_int_array_1, shape_1, group_2, parameter_0, parameter_1):
        ...
        # shape: ([S0*32, S6*S6*60]) <- ([S0*32, S6*S6*60], [S0*32, 1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([S0, 1920, S6, S6]) <- ([S0, 1920, S6, S6], [1920, 1, 1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([S0, 1920, S6, S6]) <- ([1], [S0, 1920, S6, S6])
        #  data: (None) <- ([1], None)
        add_2 = full_2 + exp_0
