...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0):
        ...
        #  type: (3xi64) <- (-1x-1x-1x320xf16)
        # shape: ([3]) <- ([S0, S3, S3, 320])
        #  data: ([S0, S3*S3, 320]) <- (None)
        generate_shape_0 = [transpose_0.shape[0], (transpose_0.shape[2] * transpose_0.shape[2]), 320] # inputs: transpose_0
        ...
        #  data: ([320]) <- (None)
        generate_shape_1 = [320] # inputs: reshape_1
