...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, matmul_0, parameter_0, group_0, parameter_1, parameter_2):
        ...
        #  type: (-1x-1x320xf16) <- (-1x-1x320xf16)
        ...
        #  type: (-1x-1x320xf16) <- (-1x-1x320xf16, -1x-1x-1xf16)
        ...
        #  type: (-1x-1x320xf32) <- (-1x-1x320xf32, -1x-1x1xf32)
        # shape: ([S0, S3*S3, 320]) <- ([S0, S3*S3, 320], [S0, S3*S3, 1])
        #  data: (None) <- (None, None)
        ...
        #  type: (1xi64) <- (-1x-1x320xf16)
        # shape: ([1]) <- ([S0, S3*S3, 320])
        #  data: ([320]) <- (None)
        generate_shape_0 = [320] # inputs: add_1
        ...
        #  type: (-1x-1x320xf32) <- (-1x-1x320xf32, -1xf32)
        # shape: ([S0, S3*S3, 320]) <- ([S0, S3*S3, 320], [320])
        #  data: (None) <- (None, None)
