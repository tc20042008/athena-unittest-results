...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0):
        ...
        # shape: ([S0, 640, (S3-1)/4+1, (S3-1)/4+1]) <- ([S0, 640, (S3-1)/4+1, (S3-1)/4+1], [1, 640, 1, 1])
        #  data: (None) <- (None, None)
        ...
        # shape: () <- ([S0, 640, (S3-1)/4+1, (S3-1)/4+1], [S0, 640, (S3-1)/4+1, (S3-1)/4+1], [S0*32, ((S3-1)/4+1)*((S3-1)/4+1)*20], [S0*32, ((S3-1)/4+1)*((S3-1)/4+1)*20])
        #  data: () <- (None, None, None, None)
        return add_0, cast_0, reshape_1, multiply_0
