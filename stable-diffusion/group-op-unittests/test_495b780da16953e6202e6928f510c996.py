...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0, parameter_1, conv2d_1, group_0):

        if FastReturn(0):
            return parameter_0, conv2d_0, parameter_1, conv2d_1, group_0
        ...
        if FastReturn(1):
            return conv2d_0, parameter_1, conv2d_1, group_0, reshape_0

        #  type: (-1x1280x-1x-1xf16) <- (-1x1280x-1x-1xf16, 1x1280x1x1xf16)
        # shape: ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1], [1, 1280, 1, 1])
        #  data: (None) <- (None, None)
        add_0 = conv2d_0 + reshape_0

        if FastReturn(2):
            return parameter_1, conv2d_1, group_0, add_0

        #  type: (1x1280x1x1xf16) <- (1280xf16)
        # shape: ([1, 1280, 1, 1]) <- ([1280])
        #  data: (None) <- (None)
        reshape_1 = paddle.reshape(parameter_1, [1, 1280, 1, 1])

        if FastReturn(3):
            return conv2d_1, group_0, add_0, reshape_1
        ...
        if FastReturn(6):
            return group_0, scale_0

        #  type: (-1x2560x-1x-1xf16) <- (-1x1280x-1x-1xf16, -1x1280x-1x-1xf16)
        # shape: ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1], [S0, 1280, (S3-1)/8+1, (S3-1)/8+1])
        #  data: (None) <- (None, None)
        concat_0 = paddle.concat([scale_0, group_0], axis=1)
        ...
        # shape: ([2]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1])
        #  data: ([S0*32, -1]) <- (None)
        ...
        # shape: ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80], [0, S0, 2560, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1], [2])
        #  data: (None, None) <- (None, [S0*32, -1])
        reshape_2, reshape_3 = paddle.reshape(cast_0, generate_shape_0), None

        if FastReturn(10):
            return concat_0, cast_0, reshape_2
        ...
        # shape: () <- ([S0, 2560, (S3-1)/8+1, (S3-1)/8+1], [S0, 2560, (S3-1)/8+1, (S3-1)/8+1], [S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80], [S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*80])
        #  data: () <- (None, None, None, None)
        return concat_0, cast_0, reshape_2, multiply_0
