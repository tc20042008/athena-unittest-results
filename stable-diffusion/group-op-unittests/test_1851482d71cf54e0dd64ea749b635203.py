...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, group_0, full_int_array_0, shape_0, group_1, full_int_array_1, shape_1, group_2, parameter_0, parameter_1):
        ...
        # shape: ([S0*32, 1]) <- ([S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*20])
        #  data: (None) <- (None)
        ...
        # shape: ([1]) <- ([2], [1, 1])
        #  data: (None) <- ([S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*20], [1])
        ...
        # shape: ([S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*20]) <- ([S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*20], [S0*32, 1])
        #  data: (None) <- (None, None)
        ...
        generate_shape_1 = [group_2.shape[0], 640, group_2.shape[3], group_2.shape[3]] # inputs: group_2
        ...
        #  type: (-1x640x-1x-1xf32) <- (-1x-1x-1x-1xf32, 640x1x1xf32)
        # shape: ([S0, 640, (S3-1)/2+1, (S3-1)/2+1]) <- ([S0, 640, (S3-1)/2+1, (S3-1)/2+1], [640, 1, 1])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x640x-1x-1xf32) <- (-1x640x-1x-1xf32, 640x1x1xf32)
        # shape: ([S0, 640, (S3-1)/2+1, (S3-1)/2+1]) <- ([S0, 640, (S3-1)/2+1, (S3-1)/2+1], [640, 1, 1])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x640x-1x-1xf32) <- (1xf32, -1x640x-1x-1xf32)
        # shape: ([S0, 640, (S3-1)/2+1, (S3-1)/2+1]) <- ([1], [S0, 640, (S3-1)/2+1, (S3-1)/2+1])
        #  data: (None) <- ([1], None)
        add_2 = full_2 + exp_0
...
class TestGroupOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            ...
            paddle.to_tensor([64, 80], dtype='int32').reshape([2]),
            paddle.uniform([64, 80], dtype='float32', min=-0.5, max=0.5),
            paddle.to_tensor([1], dtype='int64').reshape([1]),
            paddle.to_tensor([64, 80], dtype='int32').reshape([2]),
