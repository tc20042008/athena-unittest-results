...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0, group_0):
        ...
        reshape_1, reshape_2 = paddle.reshape(cast_2, generate_shape_0), None

        if FastReturn(9):
            return cast_1, cast_2, reshape_1
        ...
        #  type: () <- (-1x640x-1x-1xf16, -1x640x-1x-1xf32, -1x-1xf32, -1x-1xf32)
        # shape: () <- ([S0, 640, (S3-1)/2+1, (S3-1)/2+1], [S0, 640, (S3-1)/2+1, (S3-1)/2+1], [S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*20], [S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*20])
        #  data: () <- (None, None, None, None)
        return cast_1, cast_2, reshape_1, multiply_0
