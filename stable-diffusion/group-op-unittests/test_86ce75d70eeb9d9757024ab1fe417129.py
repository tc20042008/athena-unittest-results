...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, matmul_0, parameter_0):
        ...
        #  type: (-1x-1x2560xf16) <- (-1x-1x2560xf16, 2560xf16)
        # shape: ([S0, S3*S3, 2560]) <- ([S0, S3*S3, 2560], [2560])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x-1x1280xf16) <- (-1x-1x1280xf16, xf16)
        ...
        #  type: (-1x-1x1280xf16) <- (xf16, -1x-1x1280xf16)
        # shape: ([S0, S3*S3, 1280]) <- ([], [S0, S3*S3, 1280])
        #  data: (None) <- ([1], None)
        add_1 = full_1 + erf_0
