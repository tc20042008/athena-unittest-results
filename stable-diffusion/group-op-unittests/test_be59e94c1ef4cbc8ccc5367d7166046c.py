...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, shape_0):
        ...
        #  type: (2xi32) <- (4xi32)
        # shape: ([2]) <- ([4])
        #  data: ([(S3-1)/4+1, (S3-1)/4+1]) <- ([S0, 1280, (S3-1)/4+1, (S3-1)/4+1])
        ...
        #  type: (2xi32) <- (2xi32)
        # shape: ([2]) <- ([2])
        #  data: ([(S3-1)/4+1, (S3-1)/4+1]) <- ([(S3-1)/4+1, (S3-1)/4+1])
        cast_0 = paddle.cast(slice_0, dtype='int32')

        #  type: () <- (2xi32)
        # shape: () <- ([2])
        #  data: () <- ([(S3-1)/4+1, (S3-1)/4+1])
        return cast_0
...
class TestGroupOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            paddle.to_tensor([2, 1280, 2, 2], dtype='int32').reshape([4]),
        ]
