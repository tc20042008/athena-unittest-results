...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, matmul_0, parameter_0):
        ...
        #  type: (-1x-1x10240xf16) <- (-1x-1x10240xf16, 10240xf16)
        # shape: ([S0, ((S3-1)/8+1)*((S3-1)/8+1), 10240]) <- ([S0, ((S3-1)/8+1)*((S3-1)/8+1), 10240], [10240])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x-1x5120xf16) <- (-1x-1x5120xf16, xf16)
        ...
        #  type: (-1x-1x5120xf16) <- (xf16, -1x-1x5120xf16)
        # shape: ([S0, ((S3-1)/8+1)*((S3-1)/8+1), 5120]) <- ([], [S0, ((S3-1)/8+1)*((S3-1)/8+1), 5120])
        #  data: (None) <- ([1], None)
        add_1 = full_1 + erf_0
