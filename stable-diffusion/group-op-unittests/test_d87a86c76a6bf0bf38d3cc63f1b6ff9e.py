...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, group_0, full_int_array_0, shape_0, group_1, full_int_array_1, shape_1, group_2, parameter_0, parameter_1):
        ...
        # shape: ([S0*32, 1]) <- ([S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*10])
        #  data: (None) <- (None)
        ...
        # shape: ([1]) <- ([2], [1, 1])
        #  data: (None) <- ([S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*10], [1])
        ...
        # shape: ([S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*10]) <- ([S0*32, ((S3-1)/2+1)*((S3-1)/2+1)*10], [S0*32, 1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([S0, 320, (S3-1)/2+1, (S3-1)/2+1]) <- ([S0, 320, (S3-1)/2+1, (S3-1)/2+1], [320, 1, 1])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x320x-1x-1xf32) <- (1xf32, -1x320x-1x-1xf32)
        # shape: ([S0, 320, (S3-1)/2+1, (S3-1)/2+1]) <- ([1], [S0, 320, (S3-1)/2+1, (S3-1)/2+1])
        #  data: (None) <- ([1], None)
        add_2 = full_2 + exp_0
