...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, group_0, full_int_array_0, shape_0, group_1, full_int_array_1, shape_1, group_2, parameter_0, parameter_1):
        ...
        # shape: ([S0*32, S3*S3*30]) <- ([S0*32, S3*S3*30], [S0*32, 1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([S0, 960, S3, S3]) <- ([S0, 960, S3, S3], [960, 1, 1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([S0, 960, S3, S3]) <- ([1], [S0, 960, S3, S3])
        #  data: (None) <- ([1], None)
        add_2 = full_2 + exp_0
