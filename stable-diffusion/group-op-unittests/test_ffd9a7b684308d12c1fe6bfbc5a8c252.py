...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, matmul_0):

        if FastReturn(0):
            return matmul_0

        #  type: (-1x-1x8x160xf16) <- (-1x-1x1280xf16)
        # shape: ([S0, ((S3-1)/8+1)*((S3-1)/8+1), 8, 160]) <- ([S0, ((S3-1)/8+1)*((S3-1)/8+1), 1280])
        #  data: (None) <- (None)
        reshape_0 = paddle.reshape(matmul_0, [0, 0, 8, 160])

        #  type: () <- (-1x-1x8x160xf16)
        # shape: () <- ([S0, ((S3-1)/8+1)*((S3-1)/8+1), 8, 160])
        #  data: () <- (None)
        return reshape_0
