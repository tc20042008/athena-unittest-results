...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, matmul_0):
        ...
        #  type: (-1x-1x8x80xf16) <- (-1x-1x640xf16)
        # shape: ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 8, 80]) <- ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 640])
        #  data: (None) <- (None)
        reshape_0 = paddle.reshape(matmul_0, [0, 0, 8, 80])

        #  type: () <- (-1x-1x8x80xf16)
        # shape: () <- ([S0, ((S3-1)/2+1)*((S3-1)/2+1), 8, 80])
        #  data: () <- (None)
        return reshape_0
