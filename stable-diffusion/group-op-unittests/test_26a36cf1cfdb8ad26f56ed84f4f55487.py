...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, conv2d_0, matmul_0, parameter_1):
        ...
        # shape: ([2]) <- ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1])
        #  data: ([S0*32, -1]) <- (None)
        ...
        #  type: (-1x-1xf32, 0x-1x1280x-1x-1xi64) <- (-1x1280x-1x-1xf32, 2xi64)
        # shape: ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*40], [0, S0, 1280, (S3-1)/8+1, (S3-1)/8+1]) <- ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1], [2])
        ...
        # shape: ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*40]) <- ([S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*40], [S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*40])
        #  data: (None) <- (None, None)
        multiply_0 = reshape_2 * reshape_2

        #  type: () <- (-1x1280x-1x-1xf32, -1x-1xf32, -1x-1xf32)
        # shape: () <- ([S0, 1280, (S3-1)/8+1, (S3-1)/8+1], [S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*40], [S0*32, ((S3-1)/8+1)*((S3-1)/8+1)*40])
        #  data: () <- (None, None, None)
