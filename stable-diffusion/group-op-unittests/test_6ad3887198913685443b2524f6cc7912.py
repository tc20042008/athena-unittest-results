...
class GroupOp(paddle.nn.Layer):
    ...
    def forward(self, group_0, full_int_array_0, shape_0, group_1, full_int_array_1, shape_1, group_2, parameter_0, parameter_1):
        ...
        # shape: ([S0*32, S5*S5*60]) <- ([S0*32, S5*S5*60], [S0*32, 1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([4]) <- ([S0, 1920, S5, S5])
        #  data: ([S0, 1920, S5, S5]) <- (None)
        generate_shape_1 = [group_2.shape[0], 1920, group_2.shape[3], group_2.shape[3]] # inputs: group_2

        if FastReturn(24):
            return parameter_0, parameter_1, multiply_1, generate_shape_1

        #  type: (-1x-1x-1x-1xf32, 0x-1x-1xi64) <- (-1x-1xf32, 4xi64)
        # shape: ([S0, 1920, S5, S5], [0, S0*32, S5*S5*60]) <- ([S0*32, S5*S5*60], [4])
        #  data: (None, None) <- (None, [S0, 1920, S5, S5])
        ...
        #  type: (-1x1920x-1x-1xf32) <- (-1x-1x-1x-1xf32, 1920x1x1xf32)
        # shape: ([S0, 1920, S5, S5]) <- ([S0, 1920, S5, S5], [1920, 1, 1])
        #  data: (None) <- (None, None)
        ...
        #  type: (-1x1920x-1x-1xf32) <- (-1x1920x-1x-1xf32, 1920x1x1xf32)
        # shape: ([S0, 1920, S5, S5]) <- ([S0, 1920, S5, S5], [1920, 1, 1])
        #  data: (None) <- (None, None)
        ...
        # shape: ([S0, 1920, S5, S5]) <- ([S0, 1920, S5, S5])
        #  data: (None) <- (None)
        ...
        #  type: (-1x1920x-1x-1xf32) <- (1xf32, -1x1920x-1x-1xf32)
        # shape: ([S0, 1920, S5, S5]) <- ([1], [S0, 1920, S5, S5])
        #  data: (None) <- ([1], None)
        add_2 = full_2 + exp_0
        ...
        # shape: ([S0, 1920, S5, S5]) <- ([S0, 1920, S5, S5], [S0, 1920, S5, S5])
        #  data: (None) <- (None, None)
...
class TestGroupOp(unittest.TestCase):
    ...
    def prepare_data(self):
        self.inputs = [
            ...
            paddle.to_tensor([64, 240], dtype='int32').reshape([2]),
            paddle.uniform([64, 240], dtype='float32', min=-0.5, max=0.5),
            paddle.to_tensor([1], dtype='int64').reshape([1]),
            paddle.to_tensor([64, 240], dtype='int32').reshape([2]),
